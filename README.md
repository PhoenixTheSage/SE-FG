...
HUD sprites are not warped on the displayed interpolant: Keen draws PostPP once (FrameGen does not skip+redraw — that stacked UiBkOpacity). After Keen Present, Copy.hlsl restores HUD pixels that differ from the scene copy. If the game already fills the monitor so extra Present is skipped, there is no interpolant. Particles drawn in the scene pass still follow scene motion vectors. Camera cuts reset history. Near-zero motion (measured in the velocity buffer's own texels, before DRS→output scale) copies the current scene.
