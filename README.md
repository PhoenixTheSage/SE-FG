...
HUD sprites are not warped. Keen's PostPP prefix is skipped only while a generated frame will be inserted and that Harmony hook actually runs; Rich HUD is blended once onto a HudCopy seeded from the clean scene snapshot, then that texture opaque-replaces the backbuffer (never alpha-blended onto leftover ExtraPresent pixels). Persistents only — not merged with `BillboardsRead` clones. If the skip misses, or the game already fills the monitor so extra Present is skipped, FrameGen leaves Keen's pass alone so `UiBkOpacity` cannot stack. The extra present restores that already-composited HUD from the pixel copy. Particles drawn in the scene pass still follow scene motion vectors. Camera cuts reset history. Near-zero motion copies the current scene (no 50/50 prev+curr blend).
